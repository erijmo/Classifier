...
def main():
    ...
    for clf_name, clf in classifiers.items():
        for norm_name, norm in normalization_methods.items():
            ...
            grid_search = GridSearchCV(clf, svm_parameters, cv=5)
            grid_search.fit(X_train_normalized, y_train)
            best_svm = grid_search.best_estimator_

            # Holdout
            accuracy_holdout = evaluate_holdout(best_svm, X_train_normalized, X_test_normalized, y_train, y_test)
            results.append((clf_name, norm_name, 'Holdout', accuracy_holdout))

            # Random subsampling
            accuracy_subsampling = evaluate_random_subsampling(best_svm, X_train_normalized, y_train, X_test_normalized, y_test)
            results.append((clf_name, norm_name, 'Random subsampling', accuracy_subsampling))

            # k-fold cross-validation
            accuracy_kfold = evaluate_k_fold_cross_validation(best_svm, X_train_normalized, y_train)
            results.append((clf_name, norm_name, 'k-fold cross-validation', accuracy_kfold))

            # Leave-one-out cross-validation
            accuracy_leave_one_out = evaluate_leave_one_out(best_svm, X_train_normalized, y_train)
            results.append((clf_name, norm_name, 'Leave-one-out', accuracy_leave_one_out))

            # Bootstrap
            accuracy_bootstrap = evaluate_bootstrap(best_svm, X_train_normalized, y_train, X_test_normalized, y_test)
            results.append((clf_name, norm_name, 'Bootstrap', accuracy_bootstrap))
